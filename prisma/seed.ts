...
async function main(){
 ...
 for(const [employeeId,name,email,dep,designation,salary] of employees) await prisma.employee.upsert({where:{employeeId},update:{},create:{employeeId,name,email,designation,salary,joinedAt:new Date('2025-01-15'),departmentId:ds[dep].id}})
 const adminEmployee=await prisma.employee.findUnique({where:{employeeId:'EMP-1001'}})
 if(adminEmployee) await prisma.employee.update({where:{id:adminEmployee.id},data:{userId:user.id}})
